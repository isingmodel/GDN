...
class TimeDataset(Dataset):
    ...
    def process(self):
        ...
        ran = range(win, timestamp_len, stride) if self.mode == 'train' else range(win, timestamp_len, stride)
        data_num = len(ran)
        ...
        for cnt, i in enumerate(ran):
            ...
            for j in range(num_nodes):
                ...
                mat_i[j] = column[i - win:i]  # x data
                label_i[j] = column[i]  # label(y data): datapoint to predict
...
if __name__ == "__main__":
    ...
    train_log = train(model, config, train_dataloader, val_dataloader, nodes_list, test_dataloader, test_dataset,
